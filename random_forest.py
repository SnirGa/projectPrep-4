...
def get_rf_hyper_parameters(x, y):
    ...
    rand_search = GridSearchCV(rf, params_RF, cv=3)
...
def get_metrics(x, y):
    ...
    for train_index, test_index in kf.split(x):
        ...
        rf.predict(inference_instances)
...
def print_rf_results(csv):
    print('-----------------------'+csv+'-----------------------')
    accuracy_lst, tpr_lst, fpr_lst, precision_lst, auc_roc_curve_lst, auc_precision_recall_lst, training_time_lst, inference_time_lst = get_rf_results(csv)
